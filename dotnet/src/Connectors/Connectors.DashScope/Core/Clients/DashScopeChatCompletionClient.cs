...
/// <summary>
/// Represents a client for interacting with the chat completion DashScope model.
/// </summary>
internal sealed class DashScopeChatCompletionClient : ClientBase
{
    ...
    private static readonly string s_namespace = typeof(DashScopeChatCompletionClient).Namespace!;
    ...
    /// <summary>
    /// Represents a client for interacting with the chat completion DashScope model via GoogleAI.
    /// </summary>
    ...
    public DashScopeChatCompletionClient(
    ...
    /// <summary>
    /// Represents a client for interacting with the chat completion DashScope model via VertexAI.
    /// </summary>
    /// <param name="httpClient">HttpClient instance used to send HTTP requests</param>
    /// <param name="modelId">Id of the model supporting chat completion</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var geminiResponse = await this.SendRequestAndReturnValidDashScopeResponseAsync(
                    this._chatGenerationEndpoint, state.DashScopeRequest, cancellationToken)
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (state.Iteration = 1; ; state.Iteration++)
        {
            using var httpRequestMessage = await this.CreateHttpRequestAsync(state.DashScopeRequest, this._chatStreamingEndpoint).ConfigureAwait(false);
        ...
        PromptExecutionSettings? executionSettings)
    {
        ...
        var geminiExecutionSettings = DashScopePromptExecutionSettings.FromExecutionSettings(executionSettings);
        ...
        return new ChatCompletionState()
        {
            ...
            DashScopeRequest = CreateRequest(chatHistoryCopy, geminiExecutionSettings, kernel),
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        IAsyncEnumerator<DashScopeChatMessageContent> chatResponsesEnumerator = null!;
        try
        {
    ...
    private async Task ProcessFunctionsAsync(ChatCompletionState state, CancellationToken cancellationToken)
    {
        ...
        state.DashScopeRequest.Tools = null;
        ...
        else
        {
            ...
            state.ExecutionSettings.ToolCallBehavior!.ConfigureDashScopeRequest(state.Kernel, state.DashScopeRequest);
        }
    ...
    private async Task ProcessSingleToolCallAsync(ChatCompletionState state, DashScopeFunctionToolCall toolCall, CancellationToken cancellationToken)
    {
        ...
        if (state.ExecutionSettings.ToolCallBehavior?.AllowAnyRequestedKernelFunction is not true &&
            !IsRequestableTool(state.DashScopeRequest.Tools![0].Functions, toolCall))
        {
            this.AddToolResponseMessage(state.ChatHistory, state.DashScopeRequest, toolCall, functionResponse: null,
...
#pragma warning restore CA1031
        ...
        this.AddToolResponseMessage(state.ChatHistory, state.DashScopeRequest, toolCall,
            functionResponse: functionResult, errorMessage: null);
    }

    private async Task<DashScopeResponse> SendRequestAndReturnValidDashScopeResponseAsync(
        Uri endpoint,
        DashScopeRequest geminiRequest,
        CancellationToken cancellationToken)
    {
        ...
        string body = await this.SendRequestAndGetStringBodyAsync(httpRequestMessage, cancellationToken)
            .ConfigureAwait(false);
        var geminiResponse = DeserializeResponse<DashScopeResponse>(body);
        ValidateDashScopeResponse(geminiResponse);
        return geminiResponse;
    }

    /// <summary>Checks if a tool call is for a function that was defined.</summary>
    private static bool IsRequestableTool(IEnumerable<DashScopeTool.FunctionDeclaration> functions, DashScopeFunctionToolCall ftc)
    ...
    private void AddToolResponseMessage(
        ...
        DashScopeFunctionToolCall tool,
    ...
    private static bool CheckAutoInvokeCondition(Kernel? kernel, DashScopePromptExecutionSettings geminiExecutionSettings)
    {
        ...
        if (chatHistory.Where(message => message.Role == AuthorRole.System).ToList() is { Count: > 0 } systemMessages)
        {
            ...
            if (systemMessages.Count > 1)
            {
                throw new InvalidOperationException("Chat history can't contain more than one system message. " +
                                                    "Only the first system message will be processed but will be converted to the user message before sending to the DashScope api.");
            }
    ...
    {
        // TODO: This solution is needed due to the fact that DashScope API doesn't support system messages. Maybe in the future we will be able to remove it.
    ...
    private static void ValidateChatHistoryMessagesOrder(ChatHistory chatHistory)
    {
        ...
        ChatHistory chatHistoryCopy = new(chatHistory
            .Where(message => message.Role != AuthorRole.Tool && (message is not DashScopeChatMessageContent { ToolCalls: not null })));
        ...
        if (incorrectOrder)
        {
            throw new NotSupportedException(
                "DashScope API support only chat history with order of messages alternates between the user and the assistant. " +
                "Last message have to be User message.");
        }
    }

    private async IAsyncEnumerable<DashScopeChatMessageContent> ProcessChatResponseStreamAsync(
        Stream responseStream,
        [EnumeratorCancellation] CancellationToken ct)
    {
    ...
    private async IAsyncEnumerable<DashScopeResponse> ParseResponseStreamAsync(
        Stream responseStream,
        [EnumeratorCancellation] CancellationToken ct)
    {
        await foreach (var json in this._streamJsonParser.ParseAsync(responseStream, cancellationToken: ct).ConfigureAwait(false))
        {
            yield return DeserializeResponse<DashScopeResponse>(json);
        }
    }

    private List<DashScopeChatMessageContent> ProcessChatResponse(DashScopeResponse geminiResponse)
    {
        ValidateDashScopeResponse(geminiResponse);
    ...
    private static void ValidateDashScopeResponse(DashScopeResponse geminiResponse)
    {
        if (geminiResponse.Candidates == null || geminiResponse.Candidates.Count == 0)
        {
            if (geminiResponse.PromptFeedback?.BlockReason != null)
            {
                ...
                throw new KernelException("Prompt was blocked due to DashScope API safety reasons.");
            }

            throw new KernelException("DashScope API doesn't return any data.");
        }
    }

    private void LogUsage(List<DashScopeChatMessageContent> chatMessageContents)
        => this.LogUsageMetadata(chatMessageContents[0].Metadata!);

    private List<DashScopeChatMessageContent> GetChatMessageContentsFromResponse(DashScopeResponse geminiResponse)
        => geminiResponse.Candidates!.Select(candidate => this.GetChatMessageContentFromCandidate(geminiResponse, candidate)).ToList();

    private DashScopeChatMessageContent GetChatMessageContentFromCandidate(DashScopeResponse geminiResponse, DashScopeResponseCandidate candidate)
    {
        DashScopePart? part = candidate.Content?.Parts?[0];
        DashScopePart.FunctionCallPart[]? toolCalls = part?.FunctionCall is { } function ? [function] : null;
        return new DashScopeChatMessageContent(
    ...
    private static DashScopeRequest CreateRequest(
        ChatHistory chatHistory,
        DashScopePromptExecutionSettings geminiExecutionSettings,
        Kernel? kernel)
    {
        var geminiRequest = DashScopeRequest.FromChatHistoryAndExecutionSettings(chatHistory, geminiExecutionSettings);
        geminiExecutionSettings.ToolCallBehavior?.ConfigureDashScopeRequest(kernel, geminiRequest);
        return geminiRequest;
    }

    private DashScopeStreamingChatMessageContent GetStreamingChatContentFromChatContent(DashScopeChatMessageContent message)
    {
        if (message.CalledToolResult != null)
        {
            return new DashScopeStreamingChatMessageContent(
                role: message.Role,
    ...
    private static void ValidateAutoInvoke(bool autoInvoke, int resultsPerPrompt)
    {
        if (autoInvoke && resultsPerPrompt != 1)
        {
            ...
            throw new ArgumentException(
                $"Auto-invocation of tool calls may only be used with a {nameof(DashScopePromptExecutionSettings.CandidateCount)} of 1.");
        }
    }

    private static DashScopeMetadata GetResponseMetadata(
        DashScopeResponse geminiResponse,
        DashScopeResponseCandidate candidate) => new()
        {
    ...
    private void LogUsageMetadata(DashScopeMetadata metadata)
    {
        if (metadata.TotalTokenCount <= 0)
        {
            this.Log(LogLevel.Debug, "DashScope usage information is not available.");
            return;
        }

        this.Log(
            LogLevel.Debug,
            "DashScope usage metadata: Candidates tokens: {CandidatesTokens}, Prompt tokens: {PromptTokens}, Total tokens: {TotalTokens}",
    ...
    private sealed class ChatCompletionState
    {
        ...
        internal DashScopeRequest DashScopeRequest { get; set; } = null!;
        ...
        internal void AddLastMessageToChatHistoryAndRequest()
        {
            ...
            this.DashScopeRequest.AddChatMessage(this.LastMessage);
        }
    }
}

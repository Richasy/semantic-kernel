...
using System.Linq;
using Microsoft.SemanticKernel.Connectors.DashScope.Core;

namespace Microsoft.SemanticKernel.Connectors.DashScope;

/// <summary>Represents a behavior for DashScope tool calls.</summary>
public abstract class DashScopeToolCallBehavior
{
    ...
    // the DashScopePromptExecutionSettings has a single ToolCallBehavior property, but we could
    ...
    /// </remarks>
    public static DashScopeToolCallBehavior EnableKernelFunctions => new KernelFunctions(autoInvoke: false);
    ...
    /// If no <see cref="Kernel"/> is available, no function information will be provided to the model.
    /// </remarks>
    public static DashScopeToolCallBehavior AutoInvokeKernelFunctions => new KernelFunctions(autoInvoke: true);
    ...
    /// <returns>
    /// The <see cref="DashScopeToolCallBehavior"/> that may be set into <see cref="DashScopeToolCallBehavior"/>
    /// to indicate that the specified functions should be made available to the model.
    /// </returns>
    public static DashScopeToolCallBehavior EnableFunctions(IEnumerable<DashScopeFunction> functions, bool autoInvoke = false)
    {
    ...
    private DashScopeToolCallBehavior(bool autoInvoke)
    {
    ...
    /// <summary>Configures the <paramref name="request"/> with any tools this <see cref="DashScopeToolCallBehavior"/> provides.</summary>
    ...
    /// <param name="request">The destination <see cref="DashScopeRequest"/> to configure.</param>
    internal abstract void ConfigureDashScopeRequest(Kernel? kernel, DashScopeRequest request);

    internal DashScopeToolCallBehavior Clone()
    {
        return (DashScopeToolCallBehavior)this.MemberwiseClone();
    }

    /// <summary>
    /// Represents a <see cref="DashScopeToolCallBehavior"/> that will provide to the model all available functions from a
    /// <see cref="Kernel"/> provided by the client.
    /// </summary>
    internal sealed class KernelFunctions : DashScopeToolCallBehavior
    {
        ...
        internal override void ConfigureDashScopeRequest(Kernel? kernel, DashScopeRequest request)
        {
            ...
            {
                request.AddFunction(FunctionMetadataAsDashScopeFunction(functionMetadata));
            }
        }
        ...
        /// <summary>
        /// Convert a <see cref="KernelFunctionMetadata"/> to an <see cref="DashScopeFunction"/>.
        /// </summary>
        /// <param name="metadata">The <see cref="KernelFunctionMetadata"/> object to convert.</param>
        /// <returns>An <see cref="DashScopeFunction"/> object.</returns>
        private static DashScopeFunction FunctionMetadataAsDashScopeFunction(KernelFunctionMetadata metadata)
        {
            IReadOnlyList<KernelParameterMetadata> metadataParams = metadata.Parameters;

            var openAIParams = new DashScopeFunctionParameter[metadataParams.Count];
            for (int i = 0; i < openAIParams.Length; i++)
            {
                var param = metadataParams[i];

                openAIParams[i] = new DashScopeFunctionParameter(
                    param.Name,
            ...
            return new DashScopeFunction(
                metadata.PluginName,
                metadata.Name,
                metadata.Description,
                openAIParams,
                new DashScopeFunctionReturnParameter(
    ...
    /// <summary>
    /// Represents a <see cref="DashScopeToolCallBehavior"/> that provides a specified list of functions to the model.
    /// </summary>
    internal sealed class EnabledFunctions(IEnumerable<DashScopeFunction> functions, bool autoInvoke) : DashScopeToolCallBehavior(autoInvoke)
    {
        private readonly DashScopeFunction[] _functions = functions.ToArray();

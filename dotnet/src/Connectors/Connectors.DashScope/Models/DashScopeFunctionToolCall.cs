...
/// <summary>
/// Represents an DashScope function tool call with deserialized function name and arguments.
/// </summary>
public sealed class DashScopeFunctionToolCall
{
    private string? _fullyQualifiedFunctionName;

    /// <summary>Initialize the <see cref="DashScopeFunctionToolCall"/> from a <see cref="DashScopePart.FunctionCallPart"/>.</summary>
    internal DashScopeFunctionToolCall(DashScopePart.FunctionCallPart functionToolCall)
    {
        ...
        int separatorPos = fullyQualifiedFunctionName.IndexOf(DashScopeFunction.NameSeparator, StringComparison.Ordinal);
        if (separatorPos >= 0)
        {
            pluginName = fullyQualifiedFunctionName.AsSpan(0, separatorPos).Trim().ToString();
            functionName = fullyQualifiedFunctionName.AsSpan(separatorPos + DashScopeFunction.NameSeparator.Length).Trim().ToString();
        }
    ...
    /// separated by <see cref="DashScopeFunction.NameSeparator"/>. If there is no <see cref="PluginName"/>,
    /// this is the same as <see cref="FunctionName"/>.
    /// </remarks>
    public string FullyQualifiedName
        => this._fullyQualifiedFunctionName
            ??= string.IsNullOrEmpty(this.PluginName) ? this.FunctionName : $"{this.PluginName}{DashScopeFunction.NameSeparator}{this.FunctionName}";

...
/// <summary>Represents a behavior for SparkDesk tool calls.</summary>
public abstract class SparkDeskToolCallBehavior
{
    ...
    // the SparkDeskPromptExecutionSettings has a single ToolCallBehavior property, but we could
    ...
    /// </remarks>
    public static SparkDeskToolCallBehavior EnableKernelFunctions => new KernelFunctions(autoInvoke: false);
    ...
    /// If no <see cref="Kernel"/> is available, no function information will be provided to the model.
    /// </remarks>
    public static SparkDeskToolCallBehavior AutoInvokeKernelFunctions => new KernelFunctions(autoInvoke: true);
    ...
    private SparkDeskToolCallBehavior(bool autoInvoke)
    {
    ...
    /// <summary>Configures the <paramref name="request"/> with any tools this <see cref="SparkDeskToolCallBehavior"/> provides.</summary>
    ...
    /// <param name="request">The destination <see cref="SparkTextRequest"/> to configure.</param>
    internal abstract void ConfigureSparkTextRequest(Kernel? kernel, SparkTextRequest request);

    internal SparkDeskToolCallBehavior Clone()
    {
        return (SparkDeskToolCallBehavior)this.MemberwiseClone();
    }

    /// <summary>
    /// Represents a <see cref="SparkDeskToolCallBehavior"/> that will provide to the model all available functions from a
    /// <see cref="Kernel"/> provided by the client.
    /// </summary>
    internal sealed class KernelFunctions : SparkDeskToolCallBehavior
    {
        ...
        internal override void ConfigureSparkTextRequest(Kernel? kernel, SparkTextRequest request)
        {
            ...
            {
                request.AddFunction(FunctionMetadataAsSparkDeskFunction(functionMetadata));
            }
        }
        ...
        /// <summary>
        /// Convert a <see cref="KernelFunctionMetadata"/> to an <see cref="SparkDeskFunction"/>.
        /// </summary>
        ...
        private static SparkFunction FunctionMetadataAsSparkDeskFunction(KernelFunctionMetadata metadata)
        {
            ...
            var openAIParams = new SparkFunctionParameter[metadataParams.Count];
            for (int i = 0; i < openAIParams.Length; i++)
            {
                var param = metadataParams[i];

                openAIParams[i] = new SparkFunctionParameter(
                    param.Name,
            ...
            return new SparkFunction(
                ...
                openAIParams,
                new SparkFunctionReturnParameter(
